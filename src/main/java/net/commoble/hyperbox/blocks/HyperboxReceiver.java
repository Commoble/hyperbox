...
public enum HyperboxReceiver implements SignalReceiver
{
	INSTANCE;
	
	public static final MapCodec<HyperboxReceiver> CODEC = MapCodec.unit(INSTANCE);
	...
	{
		if (level.getBlockEntity(receiverPos) instanceof HyperboxBlockEntity hyperbox)
		{
			return hyperbox.getReceiverEndpoint(level, receiverPos, receiverState, receiverSide, connectedFace, channel);
		}
		return null;
	}
	...
	public Collection<Receiver> getAllReceivers(BlockGetter level, BlockPos receiverPos, BlockState receiverState, Channel channel)
	{
		if (level.getBlockEntity(receiverPos) instanceof HyperboxBlockEntity hyperbox)
		{
			return hyperbox.getAllReceivers(level, receiverPos, receiverState, channel);
		}
		return List.of();
	}
	...
	{
		List<HyperboxBitwiseListener> listeners = new ArrayList<>();
		for (Receiver receiver : receivers)
		{
			if (receiver instanceof HyperboxBitwiseListener listener)
			{
		...
		{
			hyperbox.resetUnusedReceivers(listeners);
		}
	}
}
